...
The function list continues to show short function names. For statistical functions, hover over the function to see the full syntax with the optional seed, for example `RandomUniform(Minimum, Maximum, [Seed])`. Clicking a statistical function inserts the same signature into the Equation Editor so the optional argument is visible while editing. Replace `[Seed]` with a numeric seed when reproducibility is wanted, or remove the optional seed argument when it is not needed.
